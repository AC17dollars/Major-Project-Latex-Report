...
\titlepreamble{Major Project Final Report (Part-A)\\On}
...
\begin{table}[H]
    ...
    \begin{tabular}{|c|c|c|c|c|L{4cm}|}
        \hline
        \multirow{2}{*}{\textbf{SN}} & \multirow{2}{*}{\textbf{Audio File}} & \textbf{Sampling} & \textbf{Duration} & \multirow{2}{*}{\textbf{Size(KiB)}} & \multirow{2}{*}{\textbf{Description}} \\
...
\subsubsection{PDF and CDF of Activations for Audio 1}
    ...
    The loss values initially start at a very low level and then experience a very steep decline, approaching zero. The loss then saturates and gradually encroaches near zero. Around epoch 300, the loss values are already near zero, but the training continues up to 1000 epochs. The reason for this extended training is that even minute improvements in the loss were positively impacting evaluation metrics like PSNR (Peak Signal-to-Noise Ratio) and log spectral distance.

    An exception is observed in the loss curve for pure tone audio at 5000 Hz. Here, the loss value falls from 0.53 to 0.499 within the first 10 epochs and then remains constant. This plateau indicates the model's difficulty in learning high-frequency audio signals.
